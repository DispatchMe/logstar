...
function log(level, output) {
  if (winstonLogger) {
    winstonLogger.log(level, output, { meta: { tags: getTags() } });
  } else {
...
const processTags = (process.env.LOGGLY_TAGS || '').split(',');

function getTags() {
  return processTags.concat(metaTags);
}

// Include these tags in the server logs
export function tags() {
  metaTags = [].slice.call(arguments);
}
